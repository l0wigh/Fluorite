#define BORDER_WIDTH			1				/* Border width around windows */
#define BORDER_FOCUSED			0x458588		/* Selected window's border color */
#define BORDER_UNFOCUSED		0x665c54		/* Selectable window's border color */
#define BORDER_INACTIVE			0x665c54	    /* Unselectable window's border color */
#define GAPS					10				/* gaps around the window */
#define STACK_OFFSET			10				/* how the stacked window are separated */
#define TOPBAR_GAPS				35				/* gaps for the top bar */
#define BOTTOMBAR_GAPS			0				/* gaps for the bottom bar */
#define	DEFAULT_MASTER_OFFSET	150				/* master window size by default */
